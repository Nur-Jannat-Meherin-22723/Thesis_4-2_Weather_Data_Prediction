...
* Then Meherin separate station 11513 data in a file named (rh_station_11513.xlsx) . 
* She handled outlier from this file (outlier_rh_11513.py and outlier_processed_rh_station_11513.xlsx). 
* Then SSA algorithm is implemented in this output file (2d_ssa_rh_11513.py and 2d_ssa_reconstructed_11513_rh.xlsx). 
* Then Nafisa analyzed some trend, seasonality, noise from this ssa reconstructed data (). 
* In the ssa reconstructed dataset Meherin has implemented Time series split ( K-fold time series split cross validation (EWKCV)----split_rh_11513.py and data in "splits_max_rh_11513" folder) for RF and SVR machine learning algorithm. 
...
* RF is applied on the dataset (2d_ssa_reconstructed_11513) in python file (rf_prediction_11513_rh.py) and the output files (test data with prediction) are in (rf_prediction_output_11513_rh folder). We've used "k fold time series cross validation" split method for k =7 for best output. The comparison output MAE, MSE, RMSE. R2 are shown in the terminal while running the python file.
