...
* She handled outlier from this file (outlier_11505.py and outlier_processed_max_temp_station_11505.xlsx). 
* Then SSA algorithm is implemented in this output file (2d_ssa_11505.py and 2d_ssa_reconstructed_11505_max_temp.xlsx). 
...
* SSA data are normalized (normalize_max_temp_11505.py and data in normalized_max_temp_station_11505.xlsx)
* Then SVR is implemented using this split (svr_max_temp_11505.py and svr_prediction_max_temp_11505 folder for output data). Shows prediction of the test folds data and performance parameters MSE, MAE, RMSE, R^2.
* RF is applied on the dataset (2d_ssa_reconstructed_11505_max_temp) in python file (rf_prediction_11505_max_temp.py) and the output files (test data with prediction) are in (rf_prediction_output_11505_max_temp folder). we've used "k fold time series cross validation" split method for k =7 for best output. The comparison output MAE, MSE, RMSE. R2 are shown in the terminal while running the python file.
