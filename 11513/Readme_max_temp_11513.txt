...
* RF is applied on the dataset (2d_ssa_reconstructed_11513_max_temp) in python file (rf_prediction_11513_max_temp.py) and the output files (test data with prediction) are in (rf_prediction_output_11513_max_temp folder). we've used "k fold time series cross validation" split method for k =7 for best output. The comparison output MAE, MSE, RMSE. R2 are shown in the terminal while running the python file.
