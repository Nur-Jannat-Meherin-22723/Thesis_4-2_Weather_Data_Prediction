...
* Then Meherin separate station 41909 data in a file named (min_temp_station_41909.xlsx) . 
* She handled outlier from this file (outlier_min_temp_41909.py and outlier_processed_min_temp_station_41909.xlsx). 
* Then SSA algorithm is implemented in this output file (2d_ssa_min_temp_41909.py and 2d_ssa_reconstructed_41909_min_temp.xlsx). 
* Then Nafisa analyzed some trend, seasonality, noise from this ssa reconstructed data (). 
* In the ssa reconstructed dataset Meherin has implemented Time series split ( K-fold time series split cross validation (EWKCV)----split_min_temp_41909.py and data in "splits_min_temp_41909" folder) for RF and SVR machine learning algorithm. 
...
* RF is applied on the dataset (2d_ssa_reconstructed_41909_min_temp) in python file (rf_prediction_41909_min_temp.py) and the output files (test data with prediction) are in (rf_prediction_output_41909_min_temp folder). we've used "k fold time series cross validation" split method for k =7 for best output. The comparison output MAE, MSE, RMSE. R2 are shown in the terminal while running the python file.
