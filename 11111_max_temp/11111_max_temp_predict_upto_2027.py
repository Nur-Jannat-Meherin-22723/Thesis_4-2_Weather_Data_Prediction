...
# Load dataset
file_path = "E:/Thesis/Web App/max_temp/11111_max_temp/2d_ssa_reconstructed_11111_max_temp.xlsx"
...
X = df[day_cols].values  # Features (daily temperatures)
years = df['Year'].values  # Target years (for filtering)

# Handle missing values
X = np.nan_to_num(X, nan=np.nanmean(X))  # Replace NaNs with column mean

# Train-test split (80% training, 20% testing)
X_train, X_test, _, _ = train_test_split(X, years, test_size=0.2, random_state=42)

# Train Random Forest model
rf = RandomForestRegressor(n_estimators=100, random_state=42)
rf.fit(X_train, X_train)  # Model learns to predict next month's daily values

# Predict future data (Sept 2021 - Dec 2027)
...
for year in predicted_years:
    for month in predicted_months:
        if year == 2021 and month < 9:
            continue  # Skip months before Sept 2021
...
# Safe file-saving
output_path = "E:/Thesis/Web App/max_temp/11111_max_temp/predicted_2d_max_temp.xlsx"
