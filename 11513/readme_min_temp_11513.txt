...
* Then Meherin separate station 11513 data in a file named (min_temp_station_11513.xlsx) . 
* She handled outlier from this file (outlier_min_temp_11513.py and outlier_processed_min_temp_station_11513.xlsx). 
...
* In the ssa reconstructed dataset Meherin has implemented Time series split ( K-fold time series split cross validation (EWKCV)----split_min_temp_11513.py and data in "splits_min_temp_11513" folder) for RF and SVR machine learning algorithm. 
...
* RF is applied on the dataset (2d_ssa_reconstructed_11513_min_temp) in python file (rf_prediction_11513_min_temp.py) and the output files (test data with prediction) are in (rf_prediction_output_11513_min_temp folder). we've used "k fold time series cross validation" split method for k =7 for best output. The comparison output MAE, MSE, RMSE. R2 are shown in the terminal while running the python file.
