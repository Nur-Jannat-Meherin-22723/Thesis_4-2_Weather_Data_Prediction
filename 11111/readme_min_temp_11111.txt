...
* Then Meherin separate station 11111 data in a file named (min_temp_station_11111.xlsx) . 
* She handled outlier from this file (outlier_11111_min_temp.py and outlier_processed_min_temp_station_11111.xlsx). 
* Then SSA algorithm is implemented in this output file (2d_ssa_min_temp_11111.py and 2d_ssa_reconstructed_11111_min_temp.xlsx). 
* Then Nafisa analyzed some trend, seasonality, noise from this ssa reconstructed data (). 
* In the ssa reconstructed dataset Meherin has implemented Time series split ( K-fold time series split cross validation (EWKCV)----split_min_temp_11111.py and data in "splits_min_temp_11111" folder) for RF and SVR machine learning algorithm. 
...
* RF is applied on the dataset (2d_ssa_reconstructed_11111_min_temp) in python file (rf_prediction_11111_min_temp.py) and the output files (test data with prediction) are in (rf_prediction_output_11111_min_temp folder). we've used "k fold time series cross validation" split method for k =7 for best output. The comparison output MAE, MSE, RMSE. R2 are shown in the terminal while running the python file.
